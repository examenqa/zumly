...
$cert = New-SelfSignedCertificate -Type Custom -Subject "CN=Zumly-Dev" `
    -KeyUsage DigitalSignature -FriendlyName "Zumly Dev" `
    -CertStoreLocation "Cert:\CurrentUser\My" `
    -TextExtension @("2.5.29.37={text}1.3.6.1.5.5.7.3.3")

# 2. Trust the certificate (allows MSIX install without Store)
Export-Certificate -Cert $cert -FilePath "$env:TEMP\Zumly-Dev.cer"
Import-Certificate -FilePath "$env:TEMP\Zumly-Dev.cer" `
    -CertStoreLocation "Cert:\CurrentUser\TrustedPeople"

# 3. Export a PFX for signing builds
Export-PfxCertificate -Cert $cert -FilePath "$env:TEMP\Zumly-Dev.pfx" `
...
.\scripts\Build-Msix.ps1 -Version "0.7.0" `
    -Publisher "CN=Zumly-Dev" `
    -LocalPfx "$env:TEMP\Zumly-Dev.pfx" `
    -PfxPassword "dev"
```
...
Zumly protects your Azure AI Foundry API keys using Windows DPAPI (Data Protection API):
...
Recording and export operations create temporary files in your system's temp directory (`%TEMP%`). Zumly implements safe cleanup:
